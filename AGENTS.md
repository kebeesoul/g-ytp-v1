...
(PROJECT SPEC = CLAUDE.md + 작업 단위 요구사항 또는 별도 spec 문서)
...
Exception:
For trivial safe fixes such as typo corrections, formatting-only changes, or clearly broken import paths, a full PROJECT SPEC is not required.
However, the agent must still explain the intended change before editing.
