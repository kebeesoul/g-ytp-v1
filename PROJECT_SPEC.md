...
| 12 | 출력 포맷 mov 고정 | **mp4 고정**. 최종 파일은 `workspace/export/{exportId}/final.mp4` |
| 13 | crossfade 2초 고정 | **`crossfadeSec` 숫자화** (스키마). v1 UI는 2초 기본값 노출 |
| 14 | 배경 fit 정책 부재 | **`fit/dim/blur` 스키마 슬롯**. v1 UI는 cover + dim 0.25 기본값 |
| 15 | 트랙 간 음량 편차 | **Target Loudness 토글 ON 시 -9 LUFS 마스터링**, OFF 시 기존 concat/render |
...
| `exportId` | Export 클릭 시 확정 | 클라이언트(editorSessionId 승격) | 영구 | DB `projects.id`, workspace/Supabase 상대 경로 기준 |
...
[음원 업로드]
     ...
     ├─ 서버: workspace/import/{editorSessionId}/track_xxx.mp3 저장
...
[배경 업로드]
     ...
     ├─ 서버: workspace/import/{editorSessionId}/bg.{ext} 저장
     └─ Background 반환

[제목 입력 + Overlay 설정 + Transition 선택]
...
[Export 클릭]
     ...
     └─ 6. 서버 백그라운드 (lib/render/runRenderPipeline.ts)
            ...
            │   ├─ STEP B: Target Loudness ON이면 트랙별 마스터링
            │   │            → workspace/tmp/{jobId}/mastered/*.wav
            │   ├─ STEP C: Phase 1 오디오 concat
            │   │            → workspace/tmp/{jobId}/concat.m4a
            │   ├─ STEP D: Phase 2 영상 합성 + 오버레이
            │   │            → workspace/export/{exportId}/final.mp4
            │   ├─ STEP E: 썸네일 추출 (640×360)
            │   │            → workspace/import/{exportId}/thumbnail.jpg
            │   ├─ STEP F: tracklist.txt 생성
            ...
            │   └─ workspace/tmp/{jobId}/ 중간 파일 정리
            │ }

[클라이언트 폴링: status='done' 수신]
     ├─ [⬇ 다운로드] 버튼 활성화 (workspace/export/{exportId}/final.mp4)
...
| 음원 파일 (재생 가능 상태) | 로컬 `workspace/import/{oldExportId}/...` | snapshot.tracks[].storagePath를 `/api/workspace-file/...`로 재로드 |
| 배경 이미지/영상 | 로컬 `workspace/import/{oldExportId}/bg.*` | snapshot.background.storagePath를 `/api/workspace-file/...`로 재로드 |
...
> `filesAvailable=false`이면 로컬 import 파일이 없다는 뜻이므로 History 편집 버튼을 비활성화하고 재업로드를 안내.
> 다운로드는 `workspace/export/{exportId}/final.mp4`가 남아있을 때만 가능. 없으면 재익스포트 진행.
...
[편집 작업 — 트랙 추가/삭제/순서 변경 등]
     ...
     │      └─ workspace/import/{editorSessionId}/track_xxx.mp3 (새 폴더)
...
[Export 클릭]
     ...
     │      │   → workspace/import/{exportId}/track_xxx.mp3 로 fs.copyFile
...
export const RenderJobRecordSchema = z.object({
  ...
  output_path: z.string().nullable(),  // workspace/export/{exportId}/final.mp4
...
```
bucket: g-ytp-v1 (Supabase Storage)
└── export/{exportId}/
    └── tracklist.txt     ← 텍스트만 유지

로컬 workspace/
├── import/{exportId}/   ← 영구 보존 (오디오, 배경, 썸네일)
│   ├── track_001.mp3
│   ├── bg.jpg | bg.mp4
│   └── thumbnail.jpg
├── export/{exportId}/   ← 영구 보존 (최종 렌더 결과)
│   └── final.mp4
└── tmp/{jobId}/         ← 휘발성 (렌더 중 임시 파일, 완료 후 cleanup)
```

오디오/배경 파일은 Supabase Storage에 업로드하지 않는다.
경로 관리는 `src/lib/workspace.ts`의 `workspacePaths` 모듈을 통해서만 접근한다.
snapshot의 storagePath는 상대 경로로 저장한다 (`import/{exportId}/filename`).
런타임 절대 경로 resolve는 `resolveStoragePath()`가 담당한다.
...
|---|---|---|---|---|
| POST | `/api/upload` | multipart + `editorSessionId` | `Track[]` | 로컬 workspace 저장 + 메타데이터 |
| POST | `/api/upload-bg` | multipart + `editorSessionId` | `Background` | 로컬 workspace 저장, mime 자동 판별 |
...
| GET | `/api/download/[jobId]` | — | mp4 스트리밍 | workspace/export/{exportId}/ |
| GET | `/api/project` | — | `ProjectRecord[]` + `filesAvailable` | status='done'만, 로컬 파일 존재 여부 포함 |
| GET | `/api/project/[exportId]` | — | `ProjectRecord` + `filesAvailable` | 단건 조회 |
...
[Export 클릭]
   ...
   │ ║   resolveStoragePath() → fs.copyFile  ║
   │ ║   → workspace/import/{exportId}/      ║
   ...
   │ ╠════════════════════════════════════════╣
   │ ║ resolveStoragePath()로 절대 경로 변환 ║
   │ ║ fileExists() 확인 (없으면 throw)      ║ ← Supabase 다운로드 없음
   ...
   │ ╠════════════════════════════════════════╣
   │ ║ Target Loudness OFF: 기본 audio config ║
   │ ║ Target Loudness ON: mastering 결과 사용 ║
   │ ║   normalize=off로 중복 정규화 방지     ║
   │ ║   → concat.m4a                        ║
   │ ║ 진행률: 0.10 ~ 0.15                    ║
   │ ╚════════════════════════════════════════╝
   │
...
[History 카드 삭제 클릭]
   ...
   │
   │ STEP 3: 로컬 import 폴더 삭제 + 검증
   │   ├─ assertInsideWorkspace() 통과 확인
   │   ├─ fs.rm(importDir, { recursive: true })
   │   └─ fs.existsSync()로 삭제 완료 검증, 남아있으면 500
   │
   │ STEP 3-b: 로컬 export 폴더 삭제
   │   └─ fs.rm(exportDir, { recursive: true })
   │      (Supabase export/{exportId}/tracklist.txt 삭제는 유지)
   │
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  // STEP 2: Supabase export folder (tracklist.txt)
...
export async function runRenderPipeline(jobId: string) {
  ...
  try {
    ...
    // 파이프라인 실행
    const snapshot = await copyImportIfNeeded(exportId, project.snapshot);
    const audioPaths = snapshot.renderConfig.mastering
      ? await masterTracksForRender(exportId, snapshot.tracks)
      : resolveTrackPaths(snapshot.tracks);
    await concatAudio(audioPaths, snapshot.renderConfig.transition);
    await renderVideo(snapshot, workspacePaths.finalVideo(exportId, "mp4"));
    await extractThumbnail(
      workspacePaths.finalVideo(exportId, "mp4"),
      workspacePaths.thumbnail(exportId)
    );
    ...
    await supabase.from("render_jobs")
      .update({
        status: "done",
        progress: 1,
        output_path: workspacePaths.finalVideo(exportId, "mp4"),
        completed_at: new Date().toISOString(),
      })
      .eq("id", jobId);
    ...
    activeProcesses.delete(jobId);
    // tmp only. Final video remains under workspace/export/{exportId}/.
...
**Step 1-2: Target Loudness 처리**

Target Loudness 토글이 켜져 있으면 각 트랙을 고정 마스터링 값으로 먼저 처리한 뒤 concat한다.
토글이 꺼져 있으면 원본 트랙을 그대로 concat하되 `AudioConfig`의 normalize 설정만 적용한다.

```bash
# OFF: 기본 normalize 예시
ffmpeg -i concat_raw.wav \
  -af loudnorm=I=-9:TP=-0.1:LRA=11:print_format=json \
  -f null - 2> loudnorm_stats.txt

# ON: mastering 결과를 사용하므로 render concat 단계에서는 중복 normalize 금지
ffmpeg -i concat_raw.wav \
  -c:a aac -b:a 384k \
  workspace/tmp/{jobId}/concat.m4a
```

> Target Loudness ON 기준은 `targetLufs=-9`, `truePeakDb=-0.1`. UI에서는 세부 파라미터를 노출하지 않는다.
> normalize=`off`인 경우 정규화 없이 바로 concat.m4a 생성.
...
| Supabase tracklist 업로드 실패 | render_jobs.status='error', error_msg 기록, 사용자 안내 |
| History 진입 시 로컬 파일 없음 | filesAvailable=false 반환, 편집 버튼 비활성화, "음원 파일 없음 — 재업로드 필요" 안내 |
| path traversal 시도 | assertInsideWorkspace() throw, 500 반환 |
| workspace 디스크 용량 부족 | fs.writeFile throw, 업로드 실패 에러 반환 |
...
| 제약 | 영향 |
|---|---|
| 오디오/배경 파일 로컬에만 저장 | Mac Studio 외 기기에서 History 복원 불가 |
| 출력 파일(mp4) 로컬에만 저장 | 다른 기기에서 다운로드 불가, workspace 파일 삭제 시 재익스포트 필요 |
| 디스크 용량 수동 관리 | 프로젝트 삭제 시 cascade 자동 정리, 고아 tmp 파일은 bootCleanup에서 정리 |
...
final.mp4는 `workspace/export/{exportId}/`에 로컬 보존된다. 단, 다른 기기나 workspace 파일이 삭제된 상태에서는 보장하지 않는다. History 진입은 `filesAvailable`로 import 파일 존재 여부를 확인하고, 마지막 Export 시점의 설정/음원/텍스트를 복원한다.

### 17.2 재익스포트 시 import 파일 처리 — **복사 방식**

snapshot 안의 storagePath들이 가리키는 모든 파일을 새 exportId 폴더로 복사한다. 각 프로젝트가 독립적 자산을 보유하므로 이전 프로젝트를 삭제해도 새 프로젝트는 깨지지 않는다. 복사는 `resolveStoragePath()` + `fs.copyFile`로 처리한다.
...
YouTube 업로드 일반성을 우선해 현재 구현은 mp4 고정이다. 최종 파일은 `workspace/export/{exportId}/final.mp4`에 저장된다. 오디오는 AAC 384kbps를 사용한다.

### 17.6 Target Loudness — **토글 방식**

Editor의 Target Loudness 토글이 켜져 있을 때만 고정 마스터링 체인을 적용한다. 기준값은 `targetLufs=-9`, `truePeakDb=-0.1`이며, 토글이 꺼져 있으면 마스터링을 바이패스한다.
...
# import/, export/, tmp/ 세 하위 디렉토리를 자동 생성
# import/, export/는 영구 보존 영역 (gitignore에 등록)
# tmp/는 렌더 완료 후 자동 cleanup
...
5. Storage bucket `g-ytp-v1` 생성 — `export/{exportId}/tracklist.txt` 용도
...
11. `/api/upload` — editorSessionId 수신, music-metadata, `workspace/import/{editorSessionId}/` 저장
12. `/api/upload-bg` — mime 판별, `workspace/import/{editorSessionId}/bg.{ext}` 저장
13. `components/editor/TitleInput.tsx`
14. `components/editor/TrackList.tsx` + `TrackItem.tsx` — dnd-kit reorder
15. `components/editor/AudioPlayer.tsx` — `/api/workspace-file/...` 기반 WaveSurfer
...
20. **`lib/ffmpeg/normalizeAudio.ts` — EBU R128 / fast normalize 지원**
...
24. `lib/ffmpeg/renderVideo.ts` — 파이프라인 오케스트레이션 (mp4 고정, h264_videotoolbox)
...
31. `/api/download/[jobId]` — `workspace/export/{exportId}/final.mp4` 스트리밍
...
34. `components/editor/RenderPanel.tsx` — 폴링, 진행률, 다운로드 버튼, Target Loudness 토글
...
42. 재익스포트 시 import 파일 복사 로직 (`resolveStoragePath()` + `fs.copyFile`)
...
| 스키마 확장 | Background fit/dim/blur, Audio normalize, Transition crossfadeSec 숫자화, Thumbnail config, mastering 토글 |
